// components/admin/CashFlowTable.tsx
'use client';
...
const CashFlowTable: React.FC<{ onDetailToggle?: (show: boolean) => void }> = ({ onDetailToggle }) => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fieldService.getCashFlow(1, 1000, 'createdDate', 'desc');
      setCashFlows(response.data.content);
    } catch (err: any) {
  ...
  useEffect(() => {
    if (onDetailToggle) {
      onDetailToggle(!!selectedCashFlow);
    }
  }, [selectedCashFlow, onDetailToggle]);

  const filteredCashFlows = useMemo(() => {
    if (!searchTerm) return cashFlows;
  ...
  const getPaginatedData = () => {
    ...
    return filteredCashFlows.slice(start, start + itemsPerPage);
  };

  const handleViewDetails = (cashFlow: CashFlow) => {
  ...
  // Show cash flow detail view when a cash flow is selected
  if (selectedCashFlow) {
    return (
      <CashFlowDetail 
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
          <div>
            ...
            <p className="text-gray-300 text-sm lg:text-base font-medium">
              Theo dõi và quản lý dòng tiền của người dùng
            </p>
          </div>
        </div>
        ...
        <div className="relative z-10">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-gray-900">Danh sách dòng tiền</h2>
                <p className="text-sm text-gray-600">{filteredCashFlows.length} total records</p>
              </div>
            </div>
            ...
            <table className="min-w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                {getPaginatedData().length === 0 ? (
                  ...
                  getPaginatedData().map((cashFlow, idx) => (
                    <tr key={idx} className="hover:bg-gray-50 transition-colors duration-150">
                      <td className="px-6 py-4 text-sm text-gray-700 max-w-[200px] truncate" title={cashFlow.email}>
                        {cashFlow.email}
                      </td>
                      ...
                      <td className="px-6 py-4 text-sm text-gray-900 font-medium">
                        {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(cashFlow.amountAvailable)}
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-900 font-medium">
                        {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(cashFlow.balance)}
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <button
                          onClick={() => handleViewDetails(cashFlow)}
